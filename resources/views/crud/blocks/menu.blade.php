@foreach($table_groups as $group)
    ...
    <div class="list-group mb-2 bg-light">
        <a href="javascript: return void(0);" class="list-group-item list-group-item-action" data-toggle="collapse" data-target="#menu{{ $group->id }}">
            <span class="text-info">
                ...
                @if(isset($table) && $table->table_group_id == $group->id)
    ...
    @isset($group->tables)
        <div class="list-group collapse mb-2 @if(isset($table) && $table->table_group_id == $group->id) show @endif list-group-collapse" data-chevron="{{$group->id}}" id="menu{{$group->id}}">
            @foreach($group->tables as $tableMenu)
                <a href="{{route('items.list', $tableMenu->url)}}" class="list-group-item list-group-item-action @if(isset($table) &&  $table->code == $tableMenu->code) bg-light @endif">
